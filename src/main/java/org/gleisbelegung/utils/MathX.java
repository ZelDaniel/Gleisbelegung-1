...
public class MathX {
    ...
    private static int maxDiv = 1; // smallest integer that 2 * maxDiv = nextCandidate
        ...
        while (nextCandidate <= i) {
            ...
            while (primes.hasNext()) {
                ...
                if (product != nextCandidate) {
                    div = nextCandidate / prime; // integer division is sufficient here
                    product = prime * div;
                }
                if (product == nextCandidate) {
                    isPrime = false;
